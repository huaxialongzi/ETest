...
public class EmmageeService extends Service {
    ...
    private String processName, packageName, startActivity, appVersion;
    private int pid, uid;
    ...
    private void createResultCsv() {
        ...
        try {
            ...
            bw.write(getString(R.string.process_package) + Constants.COMMA + packageName + Constants.LINE_END
                    ...
                    + getString(R.string.mobile_brand) + Constants.COMMA + memoryInfo.getPhoneBrand() + Constants.LINE_END
                    + getString(R.string.network_type) + Constants.COMMA + Utils.getNetworkType(this) + Constants.LINE_END
                    + getString(R.string.network_name) + Constants.COMMA + Utils.getNetworkName(this) + Constants.LINE_END
                    + "CID" + Constants.COMMA + Utils.getDeviceID(this) + Constants.LINE_END
                    + "UID" + Constants.COMMA + uid + Constants.LINE_END);

            // upload data to server
            JSONObject jsonObj = new JSONObject();
            try {
                jsonObj.put("create_time", System.currentTimeMillis());
                jsonObj.put("packageName", packageName);
                jsonObj.put("app_name", processName);
                jsonObj.put("app_version", appVersion);
                jsonObj.put("PID", pid);
                jsonObj.put("Mem", totalMemory + "MB");
                jsonObj.put("CPU", cpuInfo.getCpuName());
                jsonObj.put("sdk_version", memoryInfo.getSDKVersion());
                jsonObj.put("phone_type", memoryInfo.getPhoneType());
                jsonObj.put("brand", memoryInfo.getPhoneBrand());
                jsonObj.put("UID", uid);
                jsonObj.put("cid", Utils.getDeviceID(this));
                jsonObj.put("network_type", Utils.getNetworkType(this));
                jsonObj.put("network_name", Utils.getNetworkName(this));

                HttpUtils.createTestSuit(jsonObj.toString());
            ...
            bw.write(getString(R.string.timestamp) + Constants.COMMA
                    ...
                    + getString(R.string.traffic) + Constants.COMMA
                    + getString(R.string.battery) + Constants.COMMA
                    + getString(R.string.current) + Constants.COMMA
                    + getString(R.string.temperature) + Constants.COMMA
                    + getString(R.string.voltage) + Constants.COMMA
                    + getString(R.string.fps) + Constants.LINE_END);
        } catch (IOException e) {
...
//                                bufferedReader.reset();
    ...
    public void closeOpenedStream() {
        try {
            if (bw != null) {
                bw.write(getString(R.string.comment1) + Constants.LINE_END + getString(R.string.comment2) + Constants.LINE_END + getString(R.string.comment3) + Constants.LINE_END + getString(R.string.comment4) + Constants.LINE_END);
                bw.close();
            }
            if (osw != null)
                osw.close();
